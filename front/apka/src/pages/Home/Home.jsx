...
const Home = () => {
      ...
      const getAllNotes=async ()=>{
        try{
            ...
            if(response.data && response.data.notes){
                let tablica=(response.data.notes)
                console.log(tablica)
      ...
      const onSearchNote = async (query) => {
        try {
            const response = await axiosInstance.get("/search-notes", {
                params: { query} // Dodaj .trim() i explicite podaj parametr
            });
  ...
  return (
    <>
        <Navbar userInfo={userInfo} handleClearSearch={handleClearSearch} onSearchNote={onSearchNote}></Navbar>
        ...
        <button className='w-16 h-16 flex items-center justify-center rounded-2xl bg-blue-300 hover:bg-blue-600 absolute right-10 bottom-10' onClick={()=>{
